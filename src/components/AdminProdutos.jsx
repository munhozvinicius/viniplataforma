...
const AdminProdutos = ({ produtos, onProdutosChange }) => {
  ...
  const iniciarPublicacaoNovoProduto = () => {
    ...
    // setPopupAberto(true) [desativado]
    DataService.setConfig('produtos', (dadosParaPublicar?.produtos || produtos)).then(()=>{
      onSucessoSincronizacao(dadosParaPublicar?.produtos || produtos)
    }).catch(err=>{ console.error(err); alert('Erro ao salvar produtos.'); })
  }
